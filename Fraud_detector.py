...
if st.button("Predict"):
    ...
    log_data["probability"] = prob
    log_data["timestamp"] = datetime.now()

    file_exists = os.path.isfile("logs.csv")
    log_data.to_csv("logs.csv", mode='a', header=not file_exists, index=False)
...
if os.path.exists("logs.csv"):
    ...
    st.write("### Fraud Probability Trend")
    st.line_chart(log_df["probability"])
